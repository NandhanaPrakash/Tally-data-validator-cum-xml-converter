...
This program checks the following validations in the excel file containing Tally data
1. Checks if the voucher type is one of the following:
    1.1 Journal
    1.2 Contra
    1.3 Receipt
    1.4 Payment
...
2. Checks if the specified day is a valid day in that month (accounts for months with 30 and 31 days) and for
   February (inc leap years)

3. Checks if Amount coloumn for both Debitor and Creditor coloumns are the same amount and >0

4. Checks if Reference No starts from 1 and is incremented by 1 for consecutive rows

5. Checks if Receipt and Contra values specifies Creditor's effect first then Debitor's effect

6. Checks if Journal values specify Debitor's effect first then Creditor's effect
...
#Loading xlsx file and reading the first sheet alone
xlsx_file_path =  "C:\\Users\\Dell\\Documents\\Programming\\Simplain_Project\\voucher_data.xlsx"
...
def xlsx_to_xml(file_path, output_file_path):
...
#Function returning the no of days ibn each month
...
for index, value in df.iloc[:, 1].items():  #Index 1=coloumn 2
    if value not in allowed_voucher_values:
        invalid_voucher_cells.append((index + 2, value))  

#Checking the month coloumn for invalid values (1-12 only allowed)
invalid_month_cells = []

for index, value in df.iloc[:, 3].items():  #Index 3=4th coloumn
    if not (1 <= value <= 12):
...
for index, row in df.iterrows():
    try:
        day = int(row.iloc[2])  #Date value in the third coloumn 
        month = int(row.iloc[3])  #Month value in the fourth coloumn 
        year = row.iloc[0].year  #Year from the first coloumn 
...
for index, row in df.iterrows():
    ...
    amount1 = row.iloc[7]  #First amount coloumn H
    amount2 = row.iloc[10]  #Second amount coloumn K
...
for index, row in df.iterrows():
    ...
    voucher_type = row.iloc[1]  #Voucher Type coloumn B
    effect1 = row.iloc[6]  #First effect coloumn G
    effect2 = row.iloc[9]  #Second effect coloumn J
...
if invalid_voucher_cells or invalid_month_cells or invalid_date_cells or invalid_amount_cells or invalid_reference_cells or invalid_effect_cells:
    ...
    csv_file_path="C:\\Users\\Dell\\Downloads\\trial.csv"
    ...
    xml_file_path="C:\\Users\\Dell\\Documents\\Programming\\Simplain_Project\\voucher_data.xml"
    xlsx_to_xml(xlsx_file_path, xml_file_path)
